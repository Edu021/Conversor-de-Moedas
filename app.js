var apiKey = "85ef27e09d9cb758a441";

function juntar_string() {
    de = document.getElementById("moeda1").value
    para = document.getElementById("moeda2").value
    
    return de_para = de + "_" + para
}
function calcular(cotacao) {
    valor = document.getElementById("moeda").value
    document.getElementById("valor_convertido").innerHTML = `${valor * cotacao.toFixed(2)} ${de_para.substr(4)}`;
}

function converter(moeda1, moeda2) {
    juntar_string(moeda1, moeda2)
    ...
    .then(json=>{
        ...
        console.log(`cotacao de ${de_para}: ${cotacao}`)
        calcular(cotacao)
    })
}
...
function mostrar_cotacoes() {
    bloquearFuncao = true;
	if (!bloquearFuncao) {
		var valor_moeda_escolhida_cotacoes = document.getElementById("moeda_escolhida_cotacoes").value;
		gerar_tabela(valor_moeda_escolhida_cotacoes);
		return valor_moeda_escolhida_cotacoes
	} else {
		console.log("Função em desenvolvimento.")
	}
}

function gerar_tabela(moeda_escolhida) {
    ...
    var header_moeda2 = document.createElement("th")
    var header_moeda_text = document.createTextNode("Moeda")
    header.appendChild(header_moeda2)
    header_moeda2.appendChild(header_moeda_text)
    tblBody.appendChild(header)
    
    var header_cotacao2 = document.createElement("th")
    var header_cotacao_text = document.createTextNode(`Cotação para ${moeda_escolhida}`)
    header_cotacao2.appendChild(header_cotacao_text)
    header.appendChild(header_cotacao2)
    ...
    for (var i in moedas) {
        ...
        for (var j = 0; j < 1; j++) {
            ...
            var moeda_text = document.createTextNode(moedas[i].id);
            
            cell.appendChild(moeda_text);
            row.appendChild(cell);
        }

        // mostra valores da cotação
        for (var k = 0; k < 1; k++) {

            var cell = document.createElement("td");

            from = moedas[i].id
            var moeda_text = document.createTextNode(juntar_e_calcular_string_cotacoes());
...
function juntar_e_calcular_string_cotacoes() {
    to = document.getElementById("moeda_escolhida_cotacoes").value
    
    from_to = from + "_" + to

    fetch(`https://free.currconv.com/api/v7/convert?q=${from_to}&compact=ultra&apiKey=${apiKey}`)
    .then(src=>{
        return src.json()
    })
    .then(json=>{
        var cotacao = json[from_to]
        return cotacao
    })
...
function lista_de_moedas(){
    let url_moedas = "https://free.currconv.com/api/v7/currencies?apiKey=85ef27e09d9cb758a441"
    ...
    .then(json=>{
        ...
        var moedas = json["results"]
        matrizTeste()
        
    })
}

function matrizTeste(){
    for(var j in moedas) {
        console.log(moedas[j].id)
    }
}
